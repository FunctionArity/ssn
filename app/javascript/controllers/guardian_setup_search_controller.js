...
export default class extends Controller {
  ...
  static values = { guardians: Array, selected: Array, fieldName: String }
  ...
  filter() {
    ...
    if (matches.length === 0) {
      ...
      this.dropdownTarget.innerHTML = matches.map(g =>
        `<button type="button" class="w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700 cursor-pointer" data-action="click->guardian-setup-search#select" data-id="${g.id}" data-name="${this.escapeHtml(g.name)}">${this.escapeHtml(g.name)}</button>`
      ).join("")
    }
  ...
  addGuardian({ id, name }) {
    ...
    chip.innerHTML = `
      ...
      <button type="button" data-action="click->guardian-setup-search#remove" data-id="${id}" class="ml-1 text-green-600 hover:text-green-900 cursor-pointer">&times;</button>
    `
